...
export default function Services() {
  return (
    <section
      id="services"
      className="bg-[#F4F1EB] py-16 sm:py-24 lg:py-32"
    ...
    >
      <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-16">
        ...
        <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-4 sm:gap-6 mb-10 sm:mb-16">
          ...
          <p className="text-[0.88rem] text-[#5A6478] leading-[1.7] max-w-[38ch] font-light">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-[1px] bg-[#D0CDC6] border border-[#D0CDC6]">
          {services.map(svc => (
            <div
              key={svc.id}
              id={`service-${svc.id}`}
              className="bg-[#F4F1EB] p-7 sm:p-10 relative overflow-hidden group transition-all duration-300 hover:bg-white"
            >
              <span
                ...
                aria-hidden="true"
              />
              <div className="font-display text-[4rem] sm:text-[5rem] leading-none text-[#0057FF] opacity-[0.12] absolute top-4 right-4 sm:right-6 select-none pointer-events-none">
                {svc.num}
              </div>
              <h3 className="font-display text-[1.4rem] sm:text-[1.6rem] tracking-[0.03em] text-[#0A0A0F] leading-none mb-3 pr-12">
                {svc.label}
              </h3>
              <p className="text-[0.86rem] sm:text-[0.88rem] leading-[1.65] text-[#5A6478] mb-5 sm:mb-6 font-light max-w-[46ch]">
                {svc.description}
              </p>
              <ul className="space-y-2 mb-6 sm:mb-7" role="list">
                {svc.features.map(f => (
                  <li key={f} className="flex items-start gap-2.5 text-[0.82rem] sm:text-[0.83rem] text-[#5A6478]">
              ...
              <a
                href="#consultation"
                className="inline-flex items-center gap-1.5 text-[0.8rem] font-medium text-[#0057FF] pb-px border-b border-[#0057FF]/30 hover:border-[#0057FF] transition-colors duration-200"
