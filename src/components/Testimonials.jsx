// Testimonials — reference-style: large opening quote, Bebas Neue attribution line, clean card grid
...
export default function Testimonials() {
  return (
    <section
      id="testimonials"
      className="bg-[#D4E9FF] py-24 lg:py-32"
    ...
    >
      <div className="max-w-7xl mx-auto px-6 lg:px-16">
        ...
        {/* Cards grid */}
        <div className="grid md:grid-cols-3 gap-[1px] bg-[#b8d4ef] border border-[#b8d4ef]">
          {testimonials.map(t => (
            <figure
              key={t.id}
              className="bg-[#F4F1EB] p-8 relative overflow-hidden group hover:bg-white transition-colors duration-200"
            >
              {/* Large opening quote — reference style */}
              <span
                className="font-display text-[6rem] leading-none text-[#0057FF] opacity-[0.12] absolute top-2 left-5 select-none pointer-events-none"
                aria-hidden="true"
              >
                "
              </span>
              ...
              <blockquote className="relative mb-7">
                <p className="text-[0.92rem] leading-[1.72] text-[#0A0A0F] font-light">
                  "{t.quote}"
                </p>
              </blockquote>

              <figcaption className="border-t border-[#D0CDC6] pt-5 flex items-center gap-3">
                <div
                  className="w-9 h-9 rounded-full bg-[#0057FF] flex items-center justify-center text-white text-[0.7rem] font-bold shrink-0"
                  aria-hidden="true"
                >
                  {t.initials}
                </div>
                <div>
                  <div className="font-display text-[1rem] tracking-[0.04em] text-[#0A0A0F] leading-none">
                    {t.author}
                  </div>
                  <div
